...
public class WorldSpawner : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            var d = Instantiate(speedDrop, Camera.main.transform.position, Quaternion.identity);
            Destroy(d, 30);
            speedDropTime = Time.time + repeatTime + Random.Range(0, 15);
        }
        if (Time.time > healthDropTime)
        {
            var d = Instantiate(healthDrop, Camera.main.transform.position, Quaternion.identity);
            Destroy(d, 30);
            healthDropTime = Time.time + repeatTime + Random.Range(0, 15);
        }
        if(Time.time > bombDropTime)
        {
            var d = Instantiate(bombDrop, Camera.main.transform.position, Quaternion.identity);
            Destroy(d, 30);
            bombDropTime = Time.time + repeatTime - Random.Range(5, 15); ;
        }
        if (Time.time > enemySpawnTime)
        {
            var d = Instantiate(Enemy, Camera.main.transform.position, Quaternion.identity);
            Destroy(d, 30);
